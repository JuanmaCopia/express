...
public class DeclareVisitedSetMutator implements Mutator {
    ...
    public boolean isApplicable(Individual individual, CtCodeElement gene) {
        ...
        Set<CtTypeReference<?>> candidateTypes = SpoonQueries.getCandidateCyclicTypes(block, LocalVarHelper.SET_VAR_NAME);
    ...
    public void mutate(Individual individual, CtCodeElement gene) {
        ...
        Set<CtTypeReference<?>> candidateTypes = SpoonQueries.getCandidateCyclicTypes(blockGene, LocalVarHelper.SET_VAR_NAME);
        CtTypeReference<?> chosenType = candidateTypes.stream().findAny().get();

        CtLocalVariable<?> visitedSet = SpoonFactory.createVisitedSetDeclaration(chosenType);
