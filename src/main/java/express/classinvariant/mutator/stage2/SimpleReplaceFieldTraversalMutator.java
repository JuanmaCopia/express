...
public class SimpleReplaceFieldTraversalMutator implements ClassInvariantMutator {
    ...
    public boolean isApplicable(ClassInvariantState state) {
        ...
        List<CtIf> traversalChecks = MutatorHelper.getIfsInvokingTraversal(targetMethodBody, LocalVarHelper.STAGE_2_LABEL);
        if (traversalChecks.isEmpty())
            return false;

        checkToReplace = RandomUtils.getRandomElement(traversalChecks);
    ...
    public void mutate(ClassInvariantState state) {
        ...
        checkToReplace.replace(ifStatement);
